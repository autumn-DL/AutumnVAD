...
from trainCLS.VADCLS import VADCLS
...
def export(exp_name, ckpt_path, save_path, work_dir,wav_path):
    ...
    P1=F.interpolate(P,scale_factor=config['hop_size'],mode='linear').argmax(dim=1)
    PR=len(P1[0])
    AR=len(audio[0])
    if PR>AR:
        P2=P1[:,:AR]
    ...
    # P2=(P2==0).long()
    audx=audio*P2
    torchaudio.save(save_path,audx,config['audio_sample_rate'])
    pass
